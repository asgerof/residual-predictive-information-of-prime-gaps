#!/usr/bin/env python3
"""
Chronological alpha calibration for the B2 pair baseline.

The previous B2pair prototype used alpha=1, i.e. full truncated pair
singular-series reweighting.  That made the exact baseline code length worse
than B1.  This script scans

    B2_alpha(g | p) proportional to B1(g | p) * S({0,g}) ** alpha

where alpha=0 is B1 and alpha=1 is the naive full pair correction.

For each held-out dyadic window, alpha is selected only from earlier windows.
The script also reports the per-window oracle alpha as a diagnostic, but that
oracle row should not be interpreted as an admissible predictor.
"""
...
from rpi_b2_pair_ladder import B1Baseline, B2PairBaseline
...
def bits_for_window(events: list[GapEvent], baseline: B1Baseline | B2PairBaseline) -> float:
...
def main() -> None:
    ...
    parser.add_argument(
        "--alpha-grid",
        default="0,0.0625,0.125,0.25,0.375,0.5,0.75,1",
    )
    parser.add_argument("--out", type=Path, default=Path("experiments/results_b2_pair_alpha_scan"))
    ...
    baselines = {
        alpha: B2PairBaseline(args.b1_y, args.tuple_y, alpha)
        for alpha in alphas
    }
    ...
    write_csv(args.out / "b2_pair_alpha_grid.csv", grid_rows)
    write_csv(args.out / "b2_pair_alpha_selected.csv", selected_rows)
    (args.out / "b2_pair_alpha_metadata.json").write_text(
