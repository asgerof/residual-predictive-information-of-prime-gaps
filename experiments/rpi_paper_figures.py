#!/usr/bin/env python3
"""
Generate paper-facing SVG figures from the pinned paper-scale artifacts.
...
def save_b1_residual() -> None:
    ...
    ax.legend()
    fig.tight_layout()
    fig.savefig(OUT / "figure_1_b1_residual_stop_time.svg")
    plt.close(fig)
...
def save_ctw_control() -> None:
    ...
    ax.legend()
    fig.tight_layout()
    fig.savefig(OUT / "figure_2_ctw_positive_control.svg")
    plt.close(fig)
...
def save_baseline_ladder() -> None:
    ...
    ax.legend()
    fig.tight_layout()
    fig.savefig(OUT / "figure_3_baseline_ladder_bits.svg")
    plt.close(fig)
